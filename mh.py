...
# source from https://www.circuits.dk/testing-mh-z19-ndir-co2-sensor-module/
...
try:
    ...
    with serial.Serial(port, 9600, timeout=2.0) as ser:
        ...
        result=ser.write([0xFF,0x01,0x86,0x00,0x00,0x00,0x00,0x00,0x79])
        time.sleep(0.1)
        s=ser.read(9)
        z=bytearray(s)
        # Calculate crc
        ...
        # send "ABC logic off" command
        result=ser.write([0xFF,0x01,0x79,0x00,0x00,0x00,0x00,0x00,0x86])
        time.sleep(0.1)
        s=ser.read(9)
